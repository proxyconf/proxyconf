defmodule ProxyConf.ConfigCache do
  ...
  defp update_spec_table(cluster_id, api_id, _event) do
    case spec_from_db(cluster_id, api_id) do
      {:error, reason} ->
        Logger.error(
          cluster: cluster_id,
          api: api_id,
  ...
  defp spec_from_db(cluster_id, api_id) do
    case Db.get_spec(cluster_id, api_id) do
      nil ->
        {:error, :not_found}

      db_spec ->
        Spec.from_oas3(api_id, JSON.decode!(db_spec.data), db_spec.data)
    end
  end

  def spec_map_reduce(mapper_fn, acc, where) do
    Db.map_reduce(
      fn db_spec, acc ->
  ...
  defp cache_notify_resources(cluster_id, changed_apis) do
    {:ok, config} =
      ConfigGenerator.from_oas3_specs(&ets_spec_provider/3, cluster_id, changed_apis)
    ...
    Enum.reduce(config, config, fn {type, configs_for_type}, acc ->
      ...
      with true <- File.exists?(patch_location),
        ...
        Map.put(
          acc,
          type,
          MapPatch.patch(configs_for_type, patch) |> IO.inspect()
        )
      else
        false ->
  ...
  defp ets_spec_provider(cluster_id, acc, iterator_fn) do
    :ets.foldl(

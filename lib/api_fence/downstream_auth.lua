...
function envoy_on_request(request_handle)
  ...
  if api_id == nil or auth_type == nil or auth_field_name == nil then
    request_handle:streamInfo():dynamicMetadata():set("apifence.downstream_auth", "status", "noauth")
  ...
  if hashes then
    ...
    if client ~= nil then
      request_handle:streamInfo():dynamicMetadata():set("apifence.downstream_auth", "client_id", client.id)
      request_handle:streamInfo():dynamicMetadata():set("apifence.downstream_auth", "status", "success")
      -- auth success
      return
    else
      -- auth failed
      request_handle:streamInfo():dynamicMetadata():set("apifence.downstream_auth", "status", "failed")

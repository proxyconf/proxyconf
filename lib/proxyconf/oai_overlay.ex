defmodule ProxyConf.OaiOverlay do
  ...
  def parse(overlay) do
    ...
    {:ok,
     %ProxyConf.OaiOverlay{title: title, version: version, extends: extends, actions: actions}}
  rescue
    _ ->
  ...
  def overlay(spec_data, overlays) do
    overlays = Enum.group_by(overlays, fn %ProxyConf.OaiOverlay{extends: extends} -> extends end)
  ...
  def apply_overlay(data, overlays_for_file) do
    Enum.reduce(overlays_for_file, data, fn %ProxyConf.OaiOverlay{actions: actions}, acc ->
      Enum.reduce(actions, acc, fn
        %ProxyConf.OaiOverlay.Action{target: target, update: update, remove: false}, acc ->
          {:ok, data} =
            Warpath.update(acc, target, fn actual -> DeepMerge.deep_merge(actual, update) end)

          data

        %ProxyConf.OaiOverlay.Action{target: target, remove: true}, acc ->

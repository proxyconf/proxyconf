defmodule ProxyConf.Application do
  ...
  @server_subject "/CN=ProxyConf control plane"
  ...
  def server_cert_setup do
    server_certificate = Application.get_env(:proxyconf, :server_certificate)
    server_private_key = Application.get_env(:proxyconf, :server_private_key)
    cert_setup(@server_subject, server_certificate, server_private_key)
  end

  def client_cert_setup do
    cert_setup(@client_subject, "/tmp/proxyconf/client-cert.pem", "/tmp/proxyconf/client-key.pem")
  end
    ...
    children = [
      ...
      {GRPC.Server.Supervisor,
       ...
       cred:
         GRPC.Credential.new(
           ssl: [
             certfile: Application.get_env(:proxyconf, :server_certificate),
             keyfile: Application.get_env(:proxyconf, :server_private_key),
             cacertfile: Application.get_env(:proxyconf, :ca_certificate),
